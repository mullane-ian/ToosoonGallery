...
import {PerspectiveCamera,Environment,CubeCamera,Stars, useScroll,ScrollControls, Scroll, Preload, Image as ImageImpl, Float, Html, OrbitControls, MeshReflectorMaterial } from '@react-three/drei'
  ...
  return useFrame((state,delta) => {
    ...
    if(gamepads[0]){
      ...
      if(gamepads[0].buttons[0].pressed === true){
        gsap.to(state.camera.position, {y:10,duration:2, onComplete:setShowSecondary(false)})

      }
      if(gamepads[0].buttons[3].value >0.5){

          // setShowSecondary(false)
          gsap.to(state.camera.position, {y:0,duration:2, onComplete:setShowSecondary(false)})
...
function Mystars(){
  ...
  useFrame(()=>{
    stars.current.rotation.z += .0001
    stars.current.rotation.x -= .0001
  ...
  return(
    <>
   <Stars ref={stars} radius={3} depth={10} count={3000} factor={1} saturation={1} fade  speed={2} />
...
function Image(props) {
  ...
  useFrame((state, delta) => {
    ...
    group.current.rotation.z = THREE.MathUtils.damp(group.current.rotation.z, -0.5, 4, delta);
     }else{
    ...
    if(myState.pause === false){
      group.current.position.x -= .01
    }

    if(group.current.position.x < -115){
      if(myState.effectToggle === false){
        myState.effectToggle = true
        myState.shouldDistort=true
      }

      gsap.to( group.current.position, {x:25,duration:4,onComplete:setEffectToggle})
